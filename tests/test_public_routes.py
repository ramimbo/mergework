...
def test_public_bounties_context_preserves_limited_json_results_url() -> None:
    ...
    assert context["api_results_url"] == (
        "/api/v1/bounties?q=issue+%23580&repo=ramimbo%2Fmergework&issue_number=580&limit=25"
    )
    assert context["status_urls"]["open"] == (
        "/bounties?status=open&q=issue+%23580&repo=ramimbo%2Fmergework&issue_number=580&limit=25"
    )
